...
@DataJpaTest // 해당클래스를 JPA와 연동해 테스팅
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        /* Case 1: 4번  게시글의 모든 댓글 조회 */
        {
            ...
            // 3. 예상 데이터
            Article article =new Article(4L, "당신의 인생 영화는?", "댓글 고");//부모게시글 객체생성

            Comment a = new Comment(1L, article, "Park", "굿윌헌팅");
            Comment b = new Comment(2L, article, "Kim","아이 엠 샘");
            Comment c = new Comment(3L, article, "Jeong","아가씨");
        ...
        /* Case 2: 1번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> comments = commentRepository.findByArticleId(articleId);
            // 3. 예상 데이터
            Article article =new Article(1L, "가가가가", "1111");//부모게시글 객체생성
        ...
        /* Case 3: 9번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "9번 글 자체가 없으므로 댓글은 비어있어야함");
        }
        /* Case 4: 999번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "999번 글 자체가 없으므로 댓글은 비어있어야함");
        }
        /* Case 5: -1번 게시글의 모든 댓글 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expected.toString(), comments.toString(), "-1번 글 자체가 없으므로 댓글은 비어있어야함");
        }
    }
    ...
    void findByNickname() {
        /* Case 1: "Park"의 모든 댓글 조회 */
        {
            ...
            // 3. 예상 데이터
            Comment a = new Comment(1L, new Article(4L, "당신의 인생 영화는?", "댓글 고"), nickname,"굿윌헌팅");
        ...
        /* Case 2: "Kim"의 모든 댓글 조회 */
        {
            ...
            Comment c = new Comment(8L, new Article(6L, "당신의 취미는?", "댓글 ㄱㄱㄱ"), nickname,"유튜브 시청");
            List<Comment> expected = Arrays.asList(a,b,c);
            // 4. 비교 및 검증
